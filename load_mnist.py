...
class MNIST(data.Dataset):
    ...
    def __init__(self, root, train=True, outlier=False, out_digit=9, transform=None, target_transform=None, download=False):
        ...
        if self.train:
            ...
            self.train_labels = self.train_labels.numpy()

            indexes = np.where(self.train_labels != out_digit)
            ...
            print('total number of training dataset is %d'%(len(self.train_data)))
        else:
            self.test_data, self.test_labels = torch.load(
                os.path.join(self.root, self.processed_folder, self.test_file))
            self.test_data = self.test_data.numpy()
            self.test_labels = self.test_labels.numpy()


            if self.outlier:
                indexes = np.where(self.test_labels == out_digit)
                self.test_data = self.test_data[indexes]
            ...
            print('total number of test/outlier dataset is %d'%(len(self.test_data)))
